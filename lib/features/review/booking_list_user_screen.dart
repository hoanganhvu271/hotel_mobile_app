...
import 'package:http/http.dart' as http;
import 'dart:convert';
import '../../common/utils/api_constants.dart'; // Đảm bảo đường dẫn này đúng
...
class _BookingListUserScreenState extends State<BookingListUserScreen> {
  ...
  // State để quản lý việc hiển thị đánh giá inline
  final Set<int> _expandedBookingIds = {}; // Lưu ID của booking đang hiển thị review
  final Map<int, List<ReviewResponseDto>> _cachedReviews = {}; // Cache review đã fetch
  final Map<int, bool> _isLoadingReviews = {}; // Trạng thái loading review cho từng booking
  final Map<int, String?> _reviewFetchErrors = {}; // Lưu lỗi khi fetch review
  ...
  Future<void> _loadBookings() async {
    int? userId = await getUserId();
    if (userId != null) {
      setState(() { // Reset state trước khi fetch mới
        _bookingsFuture = null; // Để FutureBuilder hiển thị loading nếu cần
        _allBookings = [];
        _displayedBookings = [];
      ...
      _bookingsFuture = fetchBookingsWithRoomDetails(userId);
      setState(() {}); // Trigger build for loader
      _bookingsFuture!.then((data) {
        if (mounted) {
          setState(() {
            _allBookings = data;
  ...
  void _applyFiltersAndSort() {
    List<BookingWithRoomInfo> filteredList = List.from(_allBookings); // Tạo bản sao
    if (_searchQuery.isNotEmpty) {
      final query = _searchQuery.toLowerCase();
      filteredList = filteredList.where((item) {
        final bookingIdMatch = item.booking.bookingId.toString().contains(query);
  ...
  Future<void> _submitReview(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Xóa cache để fetch lại review mới và tải lại bookings để cập nhật reviewIdList
        _cachedReviews.remove(bookingId);
        _expandedBookingIds.remove(bookingId); // Đóng lại phần review nếu đang mở
        _loadBookings();
      } else {
        print(
            'Failed to submit review. Status: ${response.statusCode}, Body: ${response.body}');
    ...
    if (mounted) {
      setState(() {
        if (_expandedBookingIds.contains(bookingId)) {
          ...
          // Nếu chưa có trong cache và chưa loading thì fetch
          if (!_cachedReviews.containsKey(bookingId) && (_isLoadingReviews[bookingId] != true)) {
  ...
  Future<void> _fetchAndCacheReviews(int bookingId, List<int> reviewIds) async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _isLoadingReviews[bookingId] = false;
          _reviewFetchErrors[bookingId] = 'Không thể tải đánh giá. Vui lòng thử lại.';
        });
      }
    }
  }
  ...
  Widget _buildReviewSection(int bookingId) {
    ...
    if (_reviewFetchErrors.containsKey(bookingId) && _reviewFetchErrors[bookingId] != null) {
      return Padding(
        padding: const EdgeInsets.all(16.0),
        child: Center(
          child: Text(
            _reviewFetchErrors[bookingId]!,
    ...
    if (reviews == null || reviews.isEmpty) {
      // Trường hợp này không nên xảy ra nếu nút "Xem đánh giá" chỉ hiển thị khi có reviewId
      // nhưng vẫn để phòng trường hợp reviewIds có nhưng API trả về rỗng
      return const Padding(
        padding: EdgeInsets.symmetric(vertical: 16.0),
        child: Center(child: Text("Không tìm thấy đánh giá nào.", style: TextStyle(color: _lightTextColor))),
      );
    }
      ...
      child: Column(
        ...
        children: reviews.map((review) {
          return Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              if (reviews.indexOf(review) > 0) const Divider(height: 20, thickness: 0.5, color: Colors.grey),
              const Text("Đánh giá của bạn:", style: TextStyle(fontWeight: FontWeight.bold, color: _primaryColor)),
              const SizedBox(height: 4),
              Row(
                children: List.generate(5, (starIndex) {
                  return Icon(
              ...
              const SizedBox(height: 6),
              Text(
                review.content.isNotEmpty ? review.content : '(Không có nội dung nhận xét)',
                style: TextStyle(
                  color: review.content.isNotEmpty ? Colors.black87 : _lightTextColor,
                  fontStyle: review.content.isNotEmpty ? FontStyle.normal : FontStyle.italic,
                  fontSize: 13.5,
                ),
              ),
              const SizedBox(height: 10),
              Container(
                ...
                decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.circular(8.0),
                    border: Border.all(color: _accentColor.withOpacity(0.3), width: 0.5)
                ),
                child: Row(
                  crossAxisAlignment: CrossAxisAlignment.center,
                  children: [
                    Icon(Icons.storefront, color: _primaryColor.withOpacity(0.8), size: 20),
                    const SizedBox(width: 8),
                    const Expanded(
                      child: Text(
                        "Cảm ơn quý khách đã đánh giá!",
                        style: TextStyle(color: _primaryColor, fontStyle: FontStyle.italic, fontSize: 13),
                      ),
                    ),
                  ],
                ),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: PreferredSize(
        ...
        child: Stack(
          children: [
            const Heading(title: 'LỊCH SỬ ĐẶT PHÒNG1'),
            Positioned(
              left: 20,
              top: 22,
              child: IconButton(
              ...
              Expanded(
                child: FutureBuilder<List<BookingWithRoomInfo>>(
                  ...
                  builder: (context, snapshot) {
                    if (snapshot.connectionState == ConnectionState.waiting && _allBookings.isEmpty) {
                    ...
                    } else if (_allBookings.isEmpty && !(snapshot.connectionState == ConnectionState.waiting)) {
                      return const Center(
                        child: Text(
                          'Bạn chưa có đặt phòng nào.',
                          style: TextStyle(fontSize: 16, color: _lightTextColor),
                      ...
                      return ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final reviewIds = item.booking.reviewIdList ?? [];
                          bool hasReviews = reviewIds.isNotEmpty;
                          bool isExpanded = _expandedBookingIds.contains(booking.bookingId);
                                    ...
                                    children: [
                                      if (booking.status == 'CONFIRMED')
                                        ElevatedButton.icon(
                                          onPressed: () {
                                            if (hasReviews) {
                                              _toggleReviewDisplay(booking.bookingId, reviewIds);
                                            } else {
                                              _showReviewDialog(booking.bookingId);
                                            }
                                          },
                                          style: ElevatedButton.styleFrom(
                                            backgroundColor: (hasReviews && isExpanded)
                                                ? _accentColor // Màu khi đang mở
                                                : (hasReviews ? _accentColor.withOpacity(0.8) : _primaryColor), // Màu khi có review nhưng đóng / Màu khi chưa có review
                                            foregroundColor: Colors.white,
                                            padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
                                            shape: RoundedRectangleBorder(
                                              borderRadius: _buttonBorderRadius,
                                          ...
                                          icon: Icon(
                                            hasReviews
                                                ? (isExpanded ? Icons.keyboard_arrow_up_rounded : Icons.rate_review_outlined) // Icon thay đổi
                                          ...
                                          label: Text(
                                            hasReviews
                                                ? (isExpanded ? 'Ẩn đánh giá' : 'Xem đánh giá11') // Text thay đổi
                                                : 'Viết đánh giá11',
                                            style: const TextStyle(
                                              fontSize: 12,
                                              fontWeight: FontWeight.w600,
      ...
      child: Column(
        children: [
          TextField(
            controller: _searchController,
            decoration: InputDecoration(
              hintText: '11Tìm theo ID đặt phòng hoặc tên phòng...',
              prefixIcon: const Icon(Icons.search, color: _lightTextColor),
              suffixIcon: _searchQuery.isNotEmpty
                  ? IconButton(
                icon: const Icon(Icons.clear, color: _lightTextColor),
